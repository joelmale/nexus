...
export const DrawingTools: React.FC<DrawingToolsProps> = ({
  ...
  const createAndSyncDrawing = useCallback((drawing: Drawing) => {
  ...
  const deleteAndSyncDrawing = useCallback((drawingId: string) => {
    if (!activeScene) return;
  ...
  const getDrawingsAtPoint = useCallback((point: Point, radius: number = 5): string[] => {
    if (!activeScene) return [];
  ...
  const isPointInRectangle = (point: Point, rect: { x: number; y: number; width: number; height: number }, threshold: number = 0): boolean => {
    return point.x >= rect.x - threshold && 
  ...
  const getDrawingsInSelection = useCallback((start: Point, end: Point): string[] => {
    if (!activeScene) return [];
    ...
    return activeScene.drawings
      .filter(drawing => {
        switch (drawing.type) {
          case 'line':
            return (drawing.start.x >= minX && drawing.start.x <= maxX && 
                   drawing.start.y >= minY && drawing.start.y <= maxY) ||
                   (drawing.end.x >= minX && drawing.end.x <= maxX && 
                   drawing.end.y >= minY && drawing.end.y <= maxY);
          case 'rectangle':
            return !(drawing.x + drawing.width < minX || 
                    drawing.x > maxX || 
                    drawing.y + drawing.height < minY || 
                    drawing.y > maxY);
          case 'circle':
            return drawing.center.x >= minX && drawing.center.x <= maxX &&
                   drawing.center.y >= minY && drawing.center.y <= maxY;
          case 'pencil':
  ...
  const handleMouseUp = useCallback((e: React.MouseEvent) => {
    ...
    switch (activeTool) { 
      ...
      default: {
        ...
        const baseDrawing = {
          ...
          style: drawingStyle,
          layer: (drawingStyle.dmNotesOnly || !drawingStyle.visibleToPlayers) ? 'dm-only' : 'effects',
          createdAt: Date.now(),
          updatedAt: Date.now(),
          createdBy: user.id,
        } as const;
    ...
    e.stopPropagation();
  }, [activeTool, screenToScene, isDrawing, startPoint, selectionBox, getDrawingsInSelection, onSelectionChange, drawingStyle, user.id, createAndSyncDrawing, pencilPath]);

  // Handle polygon completion
  const handlePolygonComplete = useCallback((e: React.MouseEvent) => {
