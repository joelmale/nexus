...
function updateEnvFile(ports) {
  ...
  // Read existing .env file to preserve OAuth and other settings
  let existingEnv = {};
...
# Database
DATABASE_URL=${existingEnv.DATABASE_URL || 'postgresql://user:password@localhost:5432/nexus'}

# Google OAuth
GOOGLE_CLIENT_ID=${existingEnv.GOOGLE_CLIENT_ID || ''}
GOOGLE_CLIENT_SECRET=${existingEnv.GOOGLE_CLIENT_SECRET || ''}
GOOGLE_CALLBACK_URL=${existingEnv.GOOGLE_CALLBACK_URL || `http://localhost:${ports.websocket}/auth/google/callback`}

# Discord OAuth
DISCORD_CLIENT_ID=${existingEnv.DISCORD_CLIENT_ID || ''}
DISCORD_CLIENT_SECRET=${existingEnv.DISCORD_CLIENT_SECRET || ''}

# Session Secrets
SESSION_SECRET=${existingEnv.SESSION_SECRET || 'dev-session-secret-change-in-production'}
JWT_SECRET=${existingEnv.JWT_SECRET || 'dev-jwt-secret-change-in-production'}
`;
