...
`;
  ...
  console.log(`${colors.green}📝 Updated .env with selected ports${colors.reset}`);
}
...
class IntelligentStarter {
  ...
  async start() {
    ...
    try {
      ...
      if (portStatus.conflicts.length === 0) {
        console.log(`${colors.green}✅ All default ports are available!${colors.reset}\n`);
  ...
  async checkDefaultPorts() {
    ...
    for (const [service, port] of Object.entries(this.defaultPorts)) {
      ...
      if (isAvailable) {
        ...
        console.log(`${colors.green}✅ Port ${port} (${service}) - Available${colors.reset}`);
      } else {
        status.conflicts.push({ service, port });
        console.log(`${colors.red}❌ Port ${port} (${service}) - In Use${colors.reset}`);
      }
    }
  ...
  async handlePortConflicts(portStatus) {
    ...
    switch (choice) {
      ...
      case '3':
        ...
        break;
      case '4':
        console.log(`${colors.yellow}🛑 Startup aborted by user${colors.reset}`);
        process.exit(0);
        break;
      default:
  ...
  async autoSelectPorts() {
    console.log(`${colors.cyan}🔍 Auto-selecting available ports...${colors.reset}\n`);
    
    const ports = {
      frontend: await findAvailablePort(this.defaultPorts.frontend),
      websocket: await findAvailablePort(this.defaultPorts.websocket), 
      asset: await findAvailablePort(this.defaultPorts.asset)
    };
  ...
  async manualPortSelection(portStatus) {
    ...
    for (const conflict of portStatus.conflicts) {
      ...
      if (isNaN(portNum) || portNum < 1024 || portNum > 65535) {
        console.log(`${colors.red}Invalid port number. Using auto-selected port.${colors.reset}`);
        ports[conflict.service] = await findAvailablePort(conflict.port);
      } else if (!await checkPort(portNum)) {
        console.log(`${colors.red}Port ${portNum} is also in use. Using auto-selected port.${colors.reset}`);
  ...
  async showPortUsage(conflicts) {
    console.log(`${colors.cyan}🔍 Checking what's using the ports...${colors.reset}\n`);
    
    for (const conflict of conflicts) {
      console.log(`${colors.yellow}Port ${conflict.port} (${conflict.service}):${colors.reset}`);
    ...
    console.log(`   • Use: ${colors.bright}lsof -ti:PORT | xargs kill -9${colors.reset}`);
    console.log('');
  }
  ...
  startService(service) {
    ...
    childProcess.on('close', (code) => {
      if (!this.isShuttingDown && code !== 0) {
        console.log(`${colors.red}${service.prefix} Process crashed with code ${code}${colors.reset}`);
        this.shutdown();
      }
    });

    childProcess.on('error', (error) => {
      console.log(`${colors.red}${service.prefix} Error: ${error.message}${colors.reset}`);
      this.shutdown();
    });
  }
