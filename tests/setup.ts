...
// Mock indexedDB for tests with proper async behavior
// Global storage that persists across transactions
const globalIndexedDBStorage = new Map<string, Map<string, unknown>>();

const getOrCreateStore = (storeName: string) => {
  if (!globalIndexedDBStorage.has(storeName)) {
    globalIndexedDBStorage.set(storeName, new Map());
  }
  return globalIndexedDBStorage.get(storeName)!;
};

const mockIDBDatabase = {
  objectStoreNames: {
    contains: vi.fn((name: string) => globalIndexedDBStorage.has(name)),
    item: vi.fn((index: number) => Array.from(globalIndexedDBStorage.keys())[index] || null),
    get length() {
      return globalIndexedDBStorage.size;
    },
    [Symbol.iterator]() {
      return globalIndexedDBStorage.keys();
    },
  },
  createObjectStore: vi.fn((name: string) => {
    // Create the store in our global storage
    if (!globalIndexedDBStorage.has(name)) {
      globalIndexedDBStorage.set(name, new Map());
    }
    return {
      createIndex: vi.fn(),
    };
  ...
  transaction: vi.fn((_storeNames: string | string[], _mode?: IDBTransactionMode) => {
    const mockTransaction = {
      objectStore: vi.fn((name: string) => {
        // Get persistent storage for this store
        const storage = getOrCreateStore(name);
...
const mockIDBFactory = {
  ...
  deleteDatabase: vi.fn().mockImplementation((_name: string) => {
    const request = {
      onsuccess: null as ((event: Event) => void) | null,
      onerror: null as ((event: Event) => void) | null,
...
// Clear IndexedDB storage between tests
afterEach(() => {
  globalIndexedDBStorage.clear();
});
