...
async function hasScrollbar(page: Page, selector: string, direction: 'vertical' | 'horizontal' = 'vertical') {
  return await page.evaluate(([sel, dir]) => {
    const element = document.querySelector(sel);
...
test.describe('Panel Layout Constraints', () => {
  ...
  test('Layout panels should not exceed viewport height', async ({ page }) => {
    ...
    if (panelBounds) {
      ...
      expect(panelBounds.height).toBeLessThanOrEqual(panelBounds.viewportHeight);
  ...
  test('Scene canvas toolbar should be visible when panel has overflow content', async ({ page }) => {
    ...
    const toolbarBounds = await getViewportBounds(page, '.scene-canvas-toolbar');

    if (toolbarBounds) {
      expect(toolbarBounds.top).toBeGreaterThanOrEqual(0);
      expect(toolbarBounds.left).toBeGreaterThanOrEqual(0);
      expect(toolbarBounds.bottom).toBeLessThanOrEqual(toolbarBounds.viewportHeight);
      expect(toolbarBounds.right).toBeLessThanOrEqual(toolbarBounds.viewportWidth);
    }
  });

  test('Settings panel should be scrollable when content overflows', async ({ page }) => {
    // Open settings panel
    await page.click('[data-testid="settings-tab"]');
    await page.waitForTimeout(500);

    const settingsContent = '.settings-content';
    const hasVerticalScrollbar = await hasScrollbar(page, settingsContent, 'vertical');
  ...
  test('Sidebar resize handle should not exceed viewport height', async ({ page }) => {
    // Open any panel to show resize handle
    await page.click('[data-testid="settings-tab"]');
    await page.waitForTimeout(500);

    const resizeHandleBounds = await getViewportBounds(page, '.sidebar-resize-handle');
    ...
    if (resizeHandleBounds) {
      ...
      expect(resizeHandleBounds.bottom).toBeLessThanOrEqual(resizeHandleBounds.viewportHeight);
    }
  });

  test('Panel content should be properly constrained in different viewport sizes', async ({ page }) => {
    const viewportSizes = [
...
test.describe('Visual Layout Validation', () => {
  test('No elements should overflow the main layout container', async ({ page }) => {
    await page.goto('/game/TEST-SESSION');
    await page.waitForLoadState('networkidle');
    ...
    const overflowingElements = await page.evaluate(() => {
      ...
      for (const el of allElements) {
        ...
        if (rect.width === 0 || rect.height === 0 || el === gameLayout) continue;

        // Check if element extends beyond layout bounds
        if (rect.bottom > layoutRect.bottom + 5 || // 5px tolerance
            rect.right > layoutRect.right + 5 ||
            rect.top < layoutRect.top - 5 ||
            rect.left < layoutRect.left - 5) {

          overflowing.push({
            selector: el.tagName + (el.className ? '.' + Array.from(el.classList).join('.') : ''),
            bounds: {
              top: rect.top,
              bottom: rect.bottom,
              left: rect.left,
